...
import UrlDetailClient from "../../../../components/UrlDetailClient";
import { db } from "../../../../db";
...
import { getDomainTypeOverrides, getDomainBookmarks } from "../../../actions/brands";
...
export default async function UrlDetailPage({
  ...
  const url = sp.u ? decodeURIComponent(sp.u) : null;

  if (!url) {
    return (
      <DashboardLayout currentPath="/domains">
        <div style={{ padding: 40, color: "#94a3b8" }}>No URL specified.</div>
      </DashboardLayout>
    );
  }
  ...
  const chatFacts = allChatFacts.filter((c) =>
    c.sources.some((s) => s.url === url || s.domain.toLowerCase() === domain.toLowerCase())
  );
